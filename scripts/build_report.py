...
def _load_targeted_rerun():
    path = os.path.join(RESULTS_DIR, "leaderboard_codex_rerun_targeted.json")
    if not os.path.exists(path):
        return None
    with open(path, encoding="utf-8") as f:
        return json.load(f)
...
def build_unified(leaderboard, agent, black=None, white=None, judged=None):
    ...
    for e in (leaderboard or {}).get("models", []):
        if e.get("per_lang") and e.get("mean", {}).get("stochastic_parrot_index") is not None:
            models.append({"model": e["model"], "access_path": "chat-api",
                           "per_lang": e["per_lang"], "mean": e["mean"],
                           "classification": e.get("classification", "")})
    for p in (agent or {}).get("paths", []):
        ap = p.get("backend", "agent")
        for e in p.get("models", []):
            if e.get("mean", {}).get("stochastic_parrot_index") is not None:
...
def build_unified_md(unified):
    langs = unified["langs"]
    head = "| # | Model | Access | " + " | ".join(l.upper() for l in langs) + " | **Mean SPI** | Class |"
    sep = "|---|-------|--------|" + "|".join(["----"] * len(langs)) + "|------|-------|"
    rows = [head, sep]
    for i, e in enumerate(unified["models"], 1):
        cells = []
        for l in langs:
            v = e["per_lang"].get(l, {}).get("stochastic_parrot_index")
...
def write_docs_data(black, white, leaderboard=None, agent=None, dataset=None,
                    unified=None, judged=None, targeted_rerun=None):
    ...
    payload = {"blackbox": black, "whitebox": white,
               ...
               "agent_access": agent, "targeted_rerun": targeted_rerun,
               "dataset": dataset,
...
def main():
    ...
    agent = _load_agent()
    targeted_rerun = _load_targeted_rerun()
    ...
    write_docs_data(black, white, leaderboard, agent, dataset, unified, judged, targeted_rerun)
